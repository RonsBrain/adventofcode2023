 ...
 */
pub fn combinations(galaxies: &Vec<(i32, i32)>) -> Vec<((i32, i32), (i32, i32))>
{
	let r = 2;
    let pool: Vec<(i32, i32)> = galaxies.iter().cloned().collect();
...
fn main() {
    ...
    for (row, maybe_line) in lines.enumerate() {
        ...
        for (idx, c) in line.chars().enumerate() {
            if c == '#' {
                galaxies.push((idx as i32, row as i32));
                row_count += 1;
     ...
     * that has a row number greater than the current row. Also,
     * increase the current row by 1 to account for the expansion.
     */
    row_galaxy_count
        .iter()
        .for_each(|row_count| {
    ...
    /* For each col that has zero galaxies, move every galaxy right
     * that has a col number greater than the current row. Also,
     * increase the current col by 1 to account for the expansion.
     */
    col_galaxy_count
        .iter()
        .for_each(|col_count| {
